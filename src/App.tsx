...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0c0a2a] text-white font-sans selection:bg-[#9945FF] selection:text-white">
      {!isLearning ? (
        <>
          <Header onStartLearning={() => setIsLearning(true)} />

          <main>
            <HeroSection onStartLearning={() => setIsLearning(true)} />
            <ConstellationMap />
            <FeaturesSection />
          </main>
        ...
        <LearningModule onBack={() => setIsLearning(false)} />
      )}
    </div>
  );
}